...
const MAX_TRIGGERS = Infinity; // sem limite de eventos
...
let temAcessoGames = false;    // true se o e-mail tem a licença especial MODS_BUNDLE_ID (libera a aba "＋ Games" inteira)

// jogo_id "fantasma" em configuracoes_jogos que na verdade NÃO são jogos:
// são usados só para guardar o link de venda por canal (Discord / Site).
// Essas linhas nunca devem aparecer na grade de Exclusivos.
const HIDDEN_CONFIG_IDS = ["discord", "site"];
let discordSaleLink = ""; // link de venda cadastrado na linha "discord" do Supabase
...
function renderPresetsList() {
  ...
  countLabel.textContent = `My Presets ${state.presets.length}/6`;
...
function createPresetForGame(gameId) {
  if (state.presets.length >= 6) {
    alert("Você atingiu o limite de 6 presets desta conta.");
    return;
  }
...
function renderEditor() {
  ...
  document.getElementById("editor-limit").textContent = `Eventos: ${p.triggers.length}`;
...
function renderTriggerRow(t, idx, total) {
  ...
  let extraLine = "";
  if (evt.id === "gift") {
    const giftImg = gift ? `<img src="${gift.icon}" alt="${gift.label}" style="width:24px;height:24px;object-fit:contain;vertical-align:middle;" onerror="this.onerror=null;this.style.display='none';this.insertAdjacentText('afterend','${gift.fallback}');">` : "🎁";
    extraLine = `<div class="extra">${giftImg} ${t.count || 1} ${gift ? gift.label : "(sem presente)"}</div>`;
...
function testTrigger(id) {
  const p = activePreset();
  const t = p && p.triggers.find(x => x.id === id);
  if (!t) return;
  toast("Disparando gatilho de teste…");
  fireTikFinityTrigger(t, "Teste", null);
}
...
// Áudios de efeito — arquivos reais em assets/audio/public/
const SAMPLE_AUDIO = [
  { name: "alerta-policia.mp3", dur: "00:00.984" },
  { name: "deltarune-weapons-pull.mp3", dur: "00:00.679" },
  { name: "dragon-ball-z-heavy-punch.mp3", dur: "00:02.116" },
  { name: "efeito-sonoro-alerta.mp3", dur: "00:03.048" },
  { name: "efeitos-sonoros-brasil-sil-sil-rede-globo.mp3", dur: "00:02.900" },
  { name: "faaah.mp3", dur: "00:01.959" },
  { name: "fahhh-pump-sound.mp3", dur: "00:02.136" },
  { name: "hee-hee-levanta-pobre.mp3", dur: "00:14.524" },
  { name: "meccha-chameleon-whistle.mp3", dur: "00:00.900" },
  { name: "meow-meow-meow-tiktok.mp3", dur: "00:17.232" },
  { name: "miss_2G4oN9I.mp3", dur: "00:00.862" },
  { name: "mixkit-arcade-retro-jump-223.wav", dur: "00:00.843" },
  { name: "mixkit-arcade-retro-run-sound-220.wav", dur: "00:02.031" },
  { name: "mixkit-arcade-rising-231.wav", dur: "00:04.314" },
  { name: "mixkit-arcade-video-game-bonus-2044.wav", dur: "00:01.355" },
  { name: "mixkit-bomb-drop-impact-2804.wav", dur: "00:07.961" },
  { name: "mixkit-bonus-extra-in-a-video-game-2064.wav", dur: "00:03.922" },
  { name: "mixkit-cartoon-character-sneeze-2209.wav", dur: "00:01.109" },
  { name: "mixkit-cartoon-clown-fun-nose-sound-528.wav", dur: "00:00.594" },
  { name: "mixkit-cartoon-falling-whistle-395.wav", dur: "00:02.685" },
  { name: "mixkit-cartoon-laugh-voice-2882.wav", dur: "00:02.078" },
  { name: "mixkit-cartoon-toy-whistle-616.wav", dur: "00:01.212" },
  { name: "mixkit-clown-horn-at-circus-715.wav", dur: "00:02.480" },
  { name: "mixkit-final-level-bonus-2061.wav", dur: "00:02.590" },
  { name: "mixkit-funny-cartoon-fast-splat-2889.wav", dur: "00:01.036" },
  { name: "mixkit-funny-critter-laugh-2875.wav", dur: "00:01.345" },
  { name: "mixkit-funny-kid-voice-2879.wav", dur: "00:01.566" },
  { name: "mixkit-funny-squeaky-toy-hits-2813.wav", dur: "00:01.080" },
  { name: "mixkit-game-level-completed-2059.wav", dur: "00:03.542" },
  { name: "mixkit-human-fighter-pain-scream-2768.wav", dur: "00:01.071" },
  { name: "mixkit-little-baby-sneeze-2214.wav", dur: "00:00.780" },
  { name: "mixkit-little-boy-saying-hiya-2259.wav", dur: "00:01.146" },
  { name: "mixkit-metal-tank-gear-shift-2757.wav", dur: "00:01.287" },
  { name: "mixkit-ow-exclamation-of-pain-2204.wav", dur: "00:00.486" },
  { name: "mixkit-player-boost-recharging-2040.wav", dur: "00:01.277" },
  { name: "mixkit-police-siren-us-1643.wav", dur: "00:01.896" },
  { name: "mixkit-quick-funny-kiss-2193.wav", dur: "00:01.238" },
  { name: "mixkit-sad-game-over-trombone-471.wav", dur: "00:05.613" },
  { name: "mixkit-sci-fi-warp-slide-3113.wav", dur: "00:02.125" },
  { name: "mixkit-sweet-kitty-meow-93.wav", dur: "00:00.876" },
  { name: "mixkit-video-game-treasure-2066.wav", dur: "00:02.002" },
  { name: "mixkit-winning-a-coin-video-game-2069.wav", dur: "00:01.025" },
  { name: "ocean-meme.mp3", dur: "00:10.945" },
  { name: "oohh_chinese_man_sound_.mp3", dur: "00:02.038" },
  { name: "pikmin-gcn.mp3", dur: "00:01.646" },
  { name: "pum-impacto.mp3", dur: "00:03.213" },
  { name: "rdr2-low-honour-sound.mp3", dur: "00:03.480" },
  { name: "rizzbot-laugh.mp3", dur: "00:02.403" },
  { name: "roblox-death-sound_1.mp3", dur: "00:01.045" },
  { name: "sonic-rings-drop.mp3", dur: "00:01.933" },
];

/* ---------------- Player de áudio único (evita sobreposição de sons) ---------------- */
// Só existe UM elemento <audio> ativo no painel inteiro. Antes de tocar um som novo,
// qualquer som que esteja tocando é interrompido (pause + reset) imediatamente.
let __activeAudioEl = null;
let __activeAudioName = null;

function stopActiveAudio() {
  if (__activeAudioEl) {
    __activeAudioEl.pause();
    __activeAudioEl.currentTime = 0;
    __activeAudioEl = null;
  }
  __activeAudioName = null;
  document.querySelectorAll(".play-dot.playing").forEach(btn => {
    btn.classList.remove("playing");
    btn.textContent = "▶";
  });
}

function playEffectAudio(name, btnEl, volumePercent) {
  const wasPlayingThis = __activeAudioName === name;

  // Encerra imediatamente qualquer áudio ativo (seja este mesmo, para funcionar como toggle,
  // seja outro, para nunca haver dois sons ao mesmo tempo).
  stopActiveAudio();

  if (wasPlayingThis) return; // clique no mesmo som que já tocava = apenas para

  const audio = new Audio(`assets/audio/public/${name}`);
  audio.volume = Math.min(1, Math.max(0, (volumePercent ?? 100) / 100));
  __activeAudioEl = audio;
  __activeAudioName = name;
  if (btnEl) {
    btnEl.classList.add("playing");
    btnEl.textContent = "⏸";
  }
  audio.addEventListener("ended", () => {
    if (__activeAudioName === name) stopActiveAudio();
  });
  // Pega tanto falha na Promise de play() quanto erro de carregamento do arquivo
  // (ex: arquivo não existe em assets/audio/public/ — dá pra saber na hora em vez de ficar em silêncio).
  audio.addEventListener("error", () => {
    toast(`⚠️ Não achei o arquivo "${name}" em assets/audio/public/`);
    if (__activeAudioName === name) stopActiveAudio();
  });
  audio.play().catch(() => {
    toast(`⚠️ Não foi possível tocar "${name}"`);
    stopActiveAudio();
  });
}

// Toca o áudio configurado em um gatilho (usado ao disparar de verdade e no botão "Testar").
function playTriggerAudio(trigger) {
  if (!trigger || !trigger.audioName) {
    toast("🔇 Este gatilho não tem áudio salvo (selecione e clique em \"Salvar Gatilho\")");
    return;
  }
  playEffectAudio(trigger.audioName, null, trigger.audioVolume ?? 100);
}
...
function renderAudioList(filter) {
  ...
  document.getElementById("audio-list").innerHTML = list.map(a => `
    <div class="audio-row">
      <button class="play-dot" onclick="event.stopPropagation(); playEffectAudio('${a.name}', this)">▶</button>
...
function renderOverlaysView() {
  ...
  if (stageTriggers.length === 0) {
    ...
    stage.innerHTML = stageTriggers.map(t => {
      ...
      const evt = EVENT_TYPES.find(e => e.id === t.eventId) || EVENT_TYPES[0];
      const gift = (evt.id === "gift" && t.giftId) ? GIFT_CATALOG.find(g => g.id === t.giftId) : null;
      const iconPath = isExclusiveGame(p.gameId)
        ? exclusiveActionIconPath(p.gameId, action.id)
        : actionIconPath(p.gameId, action.id);
      const evtHtml = gift
        ? `<div class="evt-icon" title="${gift.label}"><img src="${gift.icon}" alt="${gift.label}" style="width:64px;height:64px;object-fit:contain;" onerror="this.onerror=null;this.style.display='none';this.parentElement.textContent='${gift.fallback}';"></div>`
        : `<div class="evt-icon" title="${evt.label}">${evt.icon}</div>`;
      return `
        <div class="fx">
...
function renderOverlayTriggerRow(p, t, idx, total, actionTypes) {
  ...
  let countLine = "";
  if (evt.id === "gift") {
    countLine = `${gift ? `<img src="${gift.icon}" alt="${gift.label}" style="width:20px;height:20px;object-fit:contain;vertical-align:middle;" onerror="this.onerror=null;this.style.display='none';">` : "🎁"} ${t.count || 1}`;
  } else if (evt.needsCount) {
...
async function carregarJogosExclusivos() {
  ...
  try {
    ...
    console.log("[DEBUG discord] jogo_id retornados pelo Supabase:", (jogos || []).map(j => j.jogo_id));

    // Linha "discord": não é um jogo, guarda o link de venda usado nos
    // botões de Discord do painel. Tenta os campos de URL/texto existentes,
    // na ordem mais provável de conter o link.
    const linhaDiscord = (jogos || []).find(
      j => String(j.jogo_id).toLowerCase().trim() === "discord"
    );
    console.log("[DEBUG discord] linha encontrada em configuracoes_jogos:", linhaDiscord);
    if (linhaDiscord) {
      discordSaleLink = String(
        linhaDiscord.servidor_url || linhaDiscord.descricao || linhaDiscord.link_youtube || ""
      ).trim();
    }
    console.log("[DEBUG discord] discordSaleLink calculado:", discordSaleLink);
    aplicarLinkDiscord();

    exclusiveGamesCache = (jogos || [])
      .filter(j => {
        const id = String(j.jogo_id).toLowerCase().trim();
        return id !== MODS_BUNDLE_ID && !HIDDEN_CONFIG_IDS.includes(id);
      })
      .map(j => ({
      id: j.jogo_id,
...
// Aplica o link de venda vindo do Supabase (linha "discord") em todos os
// botões de Discord do painel. Se não vier nenhum link, mantém o href
// que já está fixo no HTML (não quebra nada).
function aplicarLinkDiscord() {
  if (!discordSaleLink) return;
  document.querySelectorAll(".btn-discord, .btn-first-access").forEach(el => {
    el.setAttribute("href", discordSaleLink);
  });
}
...
async function verificarAcesso() {
  ...
  try {
    ...
    userEmail = email;
    isVerified = true;
    temAcessoGames = licencasLiberadas.includes(MODS_BUNDLE_ID);
...
function renderGamesGrid(filter) {
  ...
  const banner = document.getElementById("games-access-banner");
  if (banner) {
    banner.innerHTML = temAcessoGames
      ? `<div class="license-status active">✓ Acesso liberado — todos os mods disponíveis</div>`
      : !isVerified
        ? `<div class="license-status pending">Verifique seu e-mail para ver seu acesso</div>`
        : `<div class="license-status blocked">✕ Sem licença para a aba ＋ Games — fale com o suporte para liberar</div>`;
  }
  ...
  const locked = !temAcessoGames;

  grid.innerHTML = list.map(g => `
    <div class="game-tile exclusive-tile ${locked ? "locked" : ""}" onclick="handleGamesClick('${g.id}', ${locked})">
      ${imgWithFallback(gameCoverPath(g.id), g.name)}
      <div class="label">${g.name}</div>
      ${locked ? `<div class="license-status blocked">✕ Sem Licença</div>` : `<div class="license-status active">✓ Liberado</div>`}
    </div>`
  ).join("");
}
...
// Clique em um jogo do catálogo "＋ Games": bloqueia a aba inteira caso o
// e-mail verificado não tenha a licença especial MODS_BUNDLE_ID
function handleGamesClick(gameId, locked) {
  if (locked) {
    toast(`🔒 Sem acesso liberado à aba ＋ Games. Fale com o suporte para liberar seus mods.`);
    return;
  }
  createPresetForGame(gameId);
}
...
// jogo_id cadastrado no Supabase (tabela configuracoes_jogos) para o Arena Arena / Stick
const STICK_GAME_ID = "jogo_interativo_stick";
// Cópia local do jogo, controlado via postMessage (mesmo protocolo dos demais
// jogos exclusivos) — ver comentário completo em STICK_ACTION_TYPES (js/data.js).
const STICK_GAME_LOCAL_PATH = "mods-exclusivos/jogo_interativo_stick/index.html";
...
const LOCAL_EXCLUSIVE_GAMES = {
  ...
  [STICK_GAME_ID]: STICK_GAME_LOCAL_PATH,
};
...
// Detecta se o preset ativo é do Arena Arena / Stick
function isStickPreset(preset) {
  return preset && preset.gameId === STICK_GAME_ID;
}
...
function isCustomActionGame(preset) {
  return isPipaPreset(preset) || isSfPreset(preset) || isSquidPreset(preset) || isTruckPreset(preset) || isBladePreset(preset) || isStickPreset(preset);
}
...
function getActionsForPreset(preset) {
  ...
  if (isStickPreset(preset)) return STICK_ACTION_TYPES;
  return ACTION_TYPES;
}
...
window.renderTriggerRow = function(t, idx, total) {
  ...
  const testFn = isPipaPreset(p) ? "testPipaTrigger"
    ...
    : isStickPreset(p) ? "testStickTrigger"
    : "testSfTrigger";

  let extraLine = "";
  if (evt.id === "gift") {
    const giftImg2 = gift ? `<img src="${gift.icon}" alt="${gift.label}" style="width:24px;height:24px;object-fit:contain;vertical-align:middle;" onerror="this.onerror=null;this.style.display='none';">` : "🎁";
    extraLine = `<div class="extra">${giftImg2} ${t.count || 1} ${gift ? gift.label : "(sem presente)"}</div>`;
  } else if (evt.needsCount) {
    extraLine = `<div class="extra">${t.count || 1} Likes</div>`;
  } else if (evt.needsText) {
    extraLine = `<div class="extra">${t.text || ""}</div>`;
...
let __gameNotOpenWarned = {};
function warnGameNotOpen(key, label) {
  // Evita spammar o mesmo aviso a cada gift/comando; mostra 1x até o
  // jogo ser reaberto, e também loga no console pra facilitar debug.
  if (!__gameNotOpenWarned[key]) {
    __gameNotOpenWarned[key] = true;
    toast(`⚠️ ${label} não está aberto (ou foi fechado / a página do painel foi recarregada). Clique em "Abrir ${label}" de novo.`);
    console.warn(`[painel] ${label}: janela do jogo não encontrada, ação ignorada.`);
  }
}

function sendPipaCommand(action, user, avatar) {
  if (!pipaGameWindow || pipaGameWindow.closed) {
    warnGameNotOpen("pipa", "Pipa Interativa");
    return;
  }
  __gameNotOpenWarned.pipa = false;
...
function sendSfCommand(action, user, avatar) {
  if (!sfGameWindow || sfGameWindow.closed) {
    warnGameNotOpen("sf", "Street Fighter Interativo");
    return;
  }
  __gameNotOpenWarned.sf = false;
...
function sendSquidCommand(action, user, avatar) {
  if (!squidGameWindow || squidGameWindow.closed) {
    warnGameNotOpen("squid", "Squid Run");
    return;
  }
  __gameNotOpenWarned.squid = false;
...
function sendTruckCommand(action, user, avatar) {
  if (!truckGameWindow || truckGameWindow.closed) {
    warnGameNotOpen("truck", "Puxa Truck");
    return;
  }
  __gameNotOpenWarned.truck = false;
...
function sendBladeCommand(action, user, avatar) {
  if (!bladeGameWindow || bladeGameWindow.closed) {
    warnGameNotOpen("blade", "Blade Arena");
    return;
  }
  __gameNotOpenWarned.blade = false;
...
/* ----------- Conexão com o Arena Arena / Stick via postMessage ----------- */
// O jogo (mods-exclusivos/jogo_interativo_stick/index.html) escuta
// window.addEventListener("message", ...) esperando
// {type:"STICK_ACTION", action, user, avatar} e repassa para sua própria
// função handleAction(action, user) já existente (spawna o lutador ou
// entra na fila de espera automaticamente, igual ao Squid Run).
// Observação: o jogo também tem uma conexão TikFinity PRÓPRIA embutida
// (ws://localhost:21213/), independente do painel — ver comentário
// completo em STICK_ACTION_TYPES (js/data.js).

let stickGameWindow = null;
let stickConnected = false;

function openStickGame() {
  // O Arena Arena / Stick é um jogo embutido no próprio pacote do painel
  // (mods-exclusivos/jogo_interativo_stick/index.html) — sempre abrimos
  // essa cópia local, ignorando qualquer servidor_url vindo do Supabase.
  stickGameWindow = window.open(STICK_GAME_LOCAL_PATH, "jogo-interativo-stick",
    "width=900,height=700,resizable=yes,scrollbars=no");
  stickConnected = false;
  toast("🥊 Arena Arena aberto! Aguardando conexão…");
}

// Envia comando para o jogo
function sendStickCommand(action, user, avatar) {
  if (!stickGameWindow || stickGameWindow.closed) {
    warnGameNotOpen("stick", "Arena Arena");
    return;
  }
  __gameNotOpenWarned.stick = false;
  stickGameWindow.postMessage({ type: "STICK_ACTION", action, user, avatar }, "*");
}

// Testar gatilho do Arena Arena / Stick
function testStickTrigger(triggerId) {
  const p = activePreset();
  if (!p) return;
  const t = p.triggers.find(x => x.id === triggerId);
  if (!t) return;
  sendStickCommand(t.actionId, "TESTE_PAINEL", "https://www.tiktok.com/favicon.ico");
  toast(`▶ Testando: ${t.actionId}`);
}
...
window.renderEditor = function() {
  ...
  if (modBox.dataset.mode !== modeKey) {
    if (isPipaPreset(p)) {
      ...
      `;
    } else if (isStickPreset(p)) {
      modBox.innerHTML = `
        <div class="mod-title">Jogo Exclusivo</div>
        <button class="btn btn-green btn-sm" style="width:100%" onclick="openStickGame()">🥊 Abrir Arena Arena</button>
        <div class="help-text" id="stick-conn-status" style="margin-top:6px">Jogo fechado</div>
        <div class="help-text" style="margin-top:4px">Dica: é "Rei da Colina" 1x1 — o resto entra numa fila de espera dentro do próprio jogo. Ele também tem seu próprio painel interno de eventos TikFinity (botão ⚙️ CONFIGURAR dentro dele), independente destes gatilhos.</div>
      `;
    } else {
  ...
  // Atualiza status de conexão (Arena Arena / Stick usa postMessage)
  if (isStickPreset(p)) {
    const statusEl = document.getElementById("stick-conn-status");
    if (statusEl && !stickConnected) {
      const aberto = stickGameWindow && !stickGameWindow.closed;
      statusEl.textContent = aberto ? "⚪ Jogo aberto, aguardando conexão…" : "⚪ Jogo fechado";
      statusEl.style.color = "var(--text-dim)";
    }
  }
...
window.addEventListener("message", (evt) => {
  ...
  if (evt.data?.type === "STICK_READY") {
    stickConnected = true;
    toast("🥊 Arena Arena conectado ao painel!");
    const statusEl = document.getElementById("stick-conn-status");
    if (statusEl) {
      statusEl.textContent = "🟢 Jogo aberto e conectado";
      statusEl.style.color = "var(--green)";
...
window.createPresetForGame = function(gameId) {
  ...
  if (gameId === STICK_GAME_ID) {
    const p = state.presets[state.presets.length - 1];
    if (p && p.triggers.length === 0) {
      p.triggers = [
        { id: uid(), eventId: "follow",       actionId: "SPAWN_RANDOM", count: 1,  repetition: 1, interval: 100, delay: 0, multiplier: false, param: "", text: "", audioName: "", audioVolume: 100, giftId: null },
        { id: uid(), eventId: "like",         actionId: "HEAL_10",      count: 50, repetition: 1, interval: 100, delay: 0, multiplier: false, param: "", text: "", audioName: "", audioVolume: 100, giftId: null },
        { id: uid(), eventId: "share",        actionId: "COMBO",        count: 1,  repetition: 1, interval: 100, delay: 0, multiplier: false, param: "", text: "", audioName: "", audioVolume: 100, giftId: null },
        { id: uid(), eventId: "subscribe",    actionId: "SHOOT",        count: 1,  repetition: 1, interval: 100, delay: 0, multiplier: false, param: "", text: "", audioName: "", audioVolume: 100, giftId: null },
        { id: uid(), eventId: "chat_command", actionId: "HEAL_100",     count: 1,  repetition: 1, interval: 100, delay: 0, multiplier: false, param: "", text: "!curar", audioName: "", audioVolume: 100, giftId: null },
      ];
      saveState(state);
      renderEditor();
      toast("🥊 Gatilhos padrão do Arena Arena adicionados!");
    }
  }
};
...
function handleTikFinityEvent(ev, d, user, avatar) {
  // Se o usuário clicou em "Stop Events" no Setup, nenhuma ação deve
  // ser disparada pro jogo (antes esse botão era só visual e não
  // bloqueava nada — corrigido aqui).
  if (state.overlayEventsRunning === false) return;
...
function fireTikFinityTrigger(trigger, user, avatar) {
  const action = ACTION_TYPES.find(a => a.id === trigger.actionId)
    ...
    || (typeof SF_ACTION_TYPES !== "undefined" ? SF_ACTION_TYPES.find(a => a.id === trigger.actionId) : null)
    ...
    || (typeof STICK_ACTION_TYPES !== "undefined" ? STICK_ACTION_TYPES.find(a => a.id === trigger.actionId) : null);

  logTikFinityEvent(user, action, trigger);
  toast(`${action ? action.icon : "⚡"} ${user} → ${action ? action.label : trigger.actionId}`);

  // Toca o áudio configurado no gatilho (para qualquer sobreposição, o áudio ativo é encerrado antes)
  ...
  // Se o jogo Arena Arena / Stick estiver aberto nesta sessão, repassa a ação de verdade
  // (faltava esse bloco — por isso o Stick nunca recebia eventos reais do TikTok)
  if (stickGameWindow && !stickGameWindow.closed) {
    sendStickCommand(trigger.actionId, user, avatar);
  }
  // Se o jogo Street Fighter Interativo estiver aberto nesta sessão, repassa a ação de verdade
  // (faltava esse bloco também — usa BroadcastChannel em vez de postMessage)
  if (sfGameWindow && !sfGameWindow.closed) {
    sendSfCommand(trigger.actionId, user, avatar);
  }
}
